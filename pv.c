...
int main(int argc, char *argv[]) {
  ...
  scanf("%s", directory);
  if (*(strstr(directory, "\0") - 1) != '/' ||
      *(strstr(directory, "\0") - 1) != '\\') {
    strcat(directory, "/");
  }
  ...
  for (int i = 0; i < last; i++) {
    ...
    if (imgs[i].num == 1) {
      sprintf(cmd,
              "wget -q -O '%s%s.jpg' '--referer=http://www.pixiv.net/' "
              "https://i.pximg.net/img-master/img/%s0_master1200.jpg",
              directory, imgs[i].name, imgs[i].url);
      system(cmd);
    } else {
      for (int j = 0; j < imgs[i].num; j++) {
        memset(cmd, '\0', 280);
        sprintf(cmd,
                "wget -q -O '%s%s_%d.jpg' '--referer=http://www.pixiv.net/' "
                "https://i.pximg.net/img-master/img/%s%d_master1200.jpg",
                directory, imgs[i].name, j + 1, imgs[i].url, j);
        system(cmd);
      }
    }
  }
